 ...
 */
public class ProcDeleteTeamMember  extends HttpServlet {
        ...
        try{
            if( !DataSecurityChecker.isInsecureInputResponse(request) ) {
                ...
                if(loggedInUserBean!=null && !Utility.isNullOrEmpty(loggedInUserBean.getUserId()) ) {
                    ...
                    if(!Utility.isNullOrEmpty(sUserId) && sUserId.equalsIgnoreCase(sTeamMemberUserId)){
                        Text errorText = new ErrorText("You may not delete your own account.","err_mssg") ;
                        arrErrorText.add(errorText);

                        responseStatus = RespConstants.Status.ERROR;
                    } else if (Utility.isNullOrEmpty(sTeamMemberUserId )) {
                        ...
                        boolean isSuccessDeleting = buildUsers.deleteTeamMember(userRequestBean);
                        if( isSuccessDeleting ) {
                            jsonResponseObj.put("is_deleted",true);
                            jsonResponseObj.put("deleted_user_id", sTeamMemberUserId);
                            Text okText = new OkText("The team member was successfully deleted.","status_mssg") ;
                            arrOkText.add(okText);
                            responseStatus = RespConstants.Status.OK;
                        } else {
                            jsonResponseObj.put("is_deleted",false);
                            appLogging.info("Error while deleting Team Member Id : " + ParseUtil.checkNull(sTeamMemberUserId) );
                            Text errorText = new ErrorText("Oops!! We were unable to delete the team member you requested. Please try again later.","err_mssg") ;
                            arrErrorText.add(errorText);
