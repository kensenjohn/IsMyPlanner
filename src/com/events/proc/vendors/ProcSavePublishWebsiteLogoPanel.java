 ...
 */
public class ProcSavePublishWebsiteLogoPanel extends HttpServlet {
        ...
        try{
            if( !DataSecurityChecker.isInsecureInputResponse(request) ) {
                ...
                if(loggedInUserBean!=null && !Utility.isNullOrEmpty(loggedInUserBean.getUserId()) ) {
                    ...
                    String sLogoAction = ParseUtil.checkNull(request.getParameter("website_logo_panel_action"));
                    ...
                    String sLogoName = ParseUtil.checkNull(request.getParameter("logo_imagename"));
                    if ( Utility.isNullOrEmpty(sLogoName) ) {
                    ...
                    } else if ( Utility.isNullOrEmpty(sLogoAction) ) {
                        appLogging.info("An invalid action used by the user : " + loggedInUserBean.getUserId() );
                        Text errorText = new ErrorText("Oops!! We were unable to process your request at this time. Please try again later.(websiteLogo - 003)","err_mssg") ;
                        arrErrorText.add(errorText);
                        responseStatus = RespConstants.Status.ERROR;
                    } else {
