 ...
 */
public class ProcSortChecklistTemplateItem extends HttpServlet {
        ...
        try{
            if( !DataSecurityChecker.isInsecureInputResponse(request) ) {
                ...
                if(loggedInUserBean!=null && !Utility.isNullOrEmpty(loggedInUserBean.getUserId()) ) {
                    ...
                    if(lNumOfItems>0){
                        HashMap<String,Long> hmChecklistTemplateId = new HashMap<String, Long>();
                        for(Long lSortSequenceNumber = 0L; lSortSequenceNumber<lNumOfItems; lSortSequenceNumber++ ){
                            ...
                            hmChecklistTemplateId.put(sChecklistTemplateItemId,lSortSequenceNumber);
                        }

                        ChecklistTemplateRequestBean checklistTemplateRequestBean = new ChecklistTemplateRequestBean();
                        checklistTemplateRequestBean.setHmSortChecklistTemplateId( hmChecklistTemplateId );

                        BuildChecklistTemplate buildChecklistTemplate = new BuildChecklistTemplate();
                        buildChecklistTemplate.sortChecklistTemplateItem( checklistTemplateRequestBean );
