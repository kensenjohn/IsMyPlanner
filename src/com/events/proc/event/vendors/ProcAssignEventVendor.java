 ...
 */
public class ProcAssignEventVendor extends HttpServlet {
        ...
        try{
            if( !DataSecurityChecker.isInsecureInputResponse(request) ) {
                ...
                if(loggedInUserBean!=null && !Utility.isNullOrEmpty(loggedInUserBean.getUserId()) ) {
                    ...
                    if("assign".equalsIgnoreCase(  sAction )) {
                        EventVendorBean eventVendorBean = buildEventVendor.saveEventVendorAssignment( eventVendorRequestBean );
                        if(eventVendorBean!=null && !Utility.isNullOrEmpty(eventVendorBean.getEventVendorId())){
                            ...
                            jsonResponseObj.put("action_complete" ,"assign");
                        }
                    } else if("remove_assign".equalsIgnoreCase(  sAction )) {
                        if( buildEventVendor.deleteEventVendorAssignment( eventVendorRequestBean ) ) {
                            jsonResponseObj.put("action_complete" ,"remove_assign");
                        }
                    }
                    ...
                    Text okText = new OkText("Event Vendor changes (Assignment) are completed.","status_mssg") ;
                    arrOkText.add(okText);
                    responseStatus = RespConstants.Status.OK;

                } else {
                    appLogging.info("Invalid request in Proc Page (loggedInUserBean)" + ParseUtil.checkNullObject(loggedInUserBean) );
                    Text errorText = new ErrorText("Oops!! We were unable to process your request at this time. Please try again later.(procAssignEventVendor - 002)","err_mssg") ;
            ...
            Text errorText = new ErrorText("Oops!! We were unable to process your request at this time. Please try again later.(procAssignEventVendor - 001)","err_mssg") ;
            arrErrorText.add(errorText);
